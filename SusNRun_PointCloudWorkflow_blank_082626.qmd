...
To keep any single output from growing unmanageably large on big lidar collections, the full set of input tiles is split into batches (a configurable number of tiles each). Steps 2-4 below run once per batch -- each batch gets its own merged LAZ and (optionally) its own GeoPackage -- and the per-batch count rasters from step 4 are mosaicked into one whole-site raster before the final reprojection in step 5. Every batch is always filtered down to ground returns (Classification == 2) in step 2, whether or not the optional GeoPackage in step 3 is created -- the GeoPackage toggle only controls whether those already-ground-only points also get written out as a vector layer, not whether the filtering itself happens.

1.  Split the full set of LAZ tiles into batches of a configurable size

2.  Filter ground returns (Classification == 2) and merge LAZ tiles, one merged file per batch -- always happens, regardless of the GeoPackage option below

3.  (Optional) Write each batch's ground-only points to its own GeoPackage as a point feature class

4.  Compute a point-count raster per batch (target pixel size = 3 m), then mosaic all batches together into one whole-site count raster and derive density from it

5.  Reproject the combined density (and raw count) rasters to EPSG:2856
...
# it needs itself.
# NOTE: lasR's exec() calls already multithread internally (no explicit ncores is set below, so
# each call may use all available cores on its own). Running n_workers batches at once on top of
# that can oversubscribe the CPU -- if that's a problem, pass an explicit, lower ncores into the
# exec() calls inside filter_merge_batch/detect_native_crs/rasterize_batch_count (e.g. total
# cores / n_workers each), or lower n_workers.
n_workers <- 4

# Folder containing the laZ files
#input_dir  <- "path/to/laz_tiles"   # template -- swap in your own project's LAZ folder
input_dir <- "c:\\work\\data\\site1\\test1"  # folder holding the raw LAZ tiles for this site/date

# Output folder
#output_dir <- "path/to/output"      # template -- swap in your own output location
output_dir <- "c:\\work\\data\\site1\\test1\\grnd_den\\"
...
gpkg_layer <- "ground_points"  # layer name used inside every batch's GeoPackage
...
# if all you want is the count/density rasters, so it's off by default -- set TRUE to get a
# vector point layer per batch alongside the rasters.
export_ground_points_gpkg <- FALSE

# Final, whole-site raster outputs -- assembled by mosaicking each batch's native count raster
# together once all batches finish, then reprojecting to EPSG:2856 (see the "Combine batches"
# and reprojection sections below). The native-CRS mosaics are only kept in memory as
# intermediates on the way to these EPSG:2856 rasters, not written to disk -- these two paths
# are the only whole-site raster files the workflow produces.
...
fill_empty_cells_with_zero <- FALSE
...
# EPSG:2856 count and density rasters (count_tif_2856_path, density_tif_2856_path). Set FALSE
# to keep the per-batch files around -- e.g. to inspect them or resume a run without redoing
# batches (see filter_merge_batch above). See the "Clean up intermediate files" section near
...
Lists every input tile once, then splits that list into `batch_size_tiles`-sized groups. Everything from here on operates on one batch at a time.
...
Filters ground returns by tile, then merges one batch's tiles into a single per-batch file. This filtering always runs -- it's independent of `export_ground_points_gpkg` below, so every batch's merged LAZ is ground-only whether or not a GeoPackage is ever created for it. Defined here as a function; called once per batch in the loop below.
...
Detects the native CRS and units of a batch's merged LAZ file, needed for that batch's GeoPackage and count raster. Defined here as a function; called once per batch below.
...
Optional (controlled by `export_ground_points_gpkg` in Setup). Streams one batch's merged ground-only points (Classification == 2) into that batch's own GeoPackage. Defined here as a function; called once per batch below, only when `export_ground_points_gpkg` is TRUE.
...
Ground points only. Rasterizes one batch's ground-only points using lasR's own streaming `rasterize()` stage (not `terra::rasterize()`), which tallies points per cell while reading and never needs the point cloud loaded into R -- necessary at this file's scale (see note in the Export chunk above). Only the raw point COUNT is computed per batch; density for the whole site is derived once, after all batches' count rasters are combined (see "Combine batches" below) -- computing density per batch first and then mosaicking density values would double-count or misweight cells that straddle a batch boundary, whereas summing raw counts first and dividing once is exact. Defined here as a function; called once per batch below.
...
Calls the four functions defined above once per batch, in order, using batch-specific paths. Batches run in parallel, up to `n_workers` at a time (set in Setup), via `future.apply::future_lapply()`.
...
Mosaics every batch's native-CRS count raster into one whole-site count raster, then derives density from that combined total. Both are kept in memory only -- they're intermediates on the way to the EPSG:2856 rasters written in the next step, not final outputs, so they aren't written to disk in native CRS.
...
# Native-CRS count_rast is kept in memory only -- it's an intermediate on the way to the
# EPSG:2856 reprojection below (see "Project Density and Count Rasters to EPSG:2856"), not one
# of the final outputs, so it's not written to disk.
...
## Project Density and Count Rasters to EPSG:2856

Density is resampled bilinear (continuous field); count is resampled nearest-neighbor so cell values stay whole point counts instead of being blended by interpolation.
...
count_rast_2856 <- project(count_rast, target_crs, method = "near",
                           res = target_pixel_size_m)  # reproject the combined count raster; nearest-neighbor avoids fractional/blended counts
writeRaster(count_rast_2856, count_tif_2856_path, overwrite = TRUE)
message("Reprojected ground count raster written to: ", count_tif_2856_path)
```
...
print(count_rast_2856)    # same check for the reprojected count raster
...
Optional (controlled by `cleanup_intermediate_files` in Setup). Deletes every per-batch intermediate file -- merged LAZ, per-batch GeoPackage (if `export_ground_points_gpkg` was TRUE), per-batch native-CRS count raster, and any `.lax` spatial-index sidecar files lasR left behind in `output_dir` -- now that they've been folded into the final outputs. Only the two final EPSG:2856 raster files (`count_tif_2856_path`, `density_tif_2856_path`) are kept; the native-CRS mosaics were never written to disk in the first place (see "Combine batches" above).
...
Reports the start time, end time, and total elapsed time for the entire workflow, using the `workflow_start_time` timestamp captured at the very top of the Setup chunk.
